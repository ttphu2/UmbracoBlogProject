...
namespace CleanBlog.Core.Controllers
{
    public class SearchController : RenderMvcController
    {
        ...
        public SearchController(ISearchService searchService, IDataTypeValueService dataTypeValueService)
        {
            _searchService = searchService;
            _dataTypeValueService = dataTypeValueService;
            DocTypeAliases = new[] { "article", "content" };
        }
        public ActionResult Index(ContentModel model,string query,string page,string category)
        {
            ...
            IEnumerable<SelectListItem> categories 
                = _dataTypeValueService.GetItemsFromValueListDataType("[MULTI CHECKBOX] Cagetory List",null);
